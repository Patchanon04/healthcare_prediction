...
from .models import Transaction, Patient, SecondOpinionRequest
...
class SecondOpinionRequestTestCase(TestCase):
    """Tests for second opinion request lifecycle."""

    def setUp(self):
        self.client = APIClient()
        self.requester = User.objects.create_user(username='requester', password='testpass')
        self.specialist = User.objects.create_user(username='specialist', password='testpass')
    ...
    def test_requester_can_create_request(self):
        self.client.force_authenticate(user=self.requester)
        payload = {
            'patient': self.patient.id,
            'diagnosis': str(self.transaction.id),
            'assignee': self.specialist.id,
            'question': 'Please review the MRI for any signs of progression.',
            'due_at': timezone.now().isoformat()
        }

        response = self.client.post('/api/v1/second-opinions/', payload, format='json')

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.json()['status'], SecondOpinionRequest.STATUS_PENDING)
        self.assertEqual(SecondOpinionRequest.objects.count(), 1)

    def test_only_requester_or_assignee_can_view(self):
        request_obj = SecondOpinionRequest.objects.create(
            requester=self.requester,
            assignee=self.specialist,
            patient=self.patient,
            diagnosis=self.transaction,
            question='Check white matter.',
        )

        # requester can view
        self.client.force_authenticate(user=self.requester)
        resp = self.client.get(f'/api/v1/second-opinions/{request_obj.id}/')
    ...
    def test_specialist_can_complete_with_response(self):
        request_obj = SecondOpinionRequest.objects.create(
            requester=self.requester,
            assignee=self.specialist,
            patient=self.patient,
            diagnosis=self.transaction,
            question='Confirm the lesion type.',
        )

        self.client.force_authenticate(user=self.specialist)
        payload = {
            'status': SecondOpinionRequest.STATUS_COMPLETED,
            'response': 'Imaging suggests stable low-grade glioma. Recommend follow-up in 6 months.'
        }

        response = self.client.patch(f'/api/v1/second-opinions/{request_obj.id}/', payload, format='json')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        request_obj.refresh_from_db()
        self.assertEqual(request_obj.status, SecondOpinionRequest.STATUS_COMPLETED)
        self.assertTrue(request_obj.responded_at)
        self.assertIn('stable low-grade glioma', request_obj.response)

    def test_cannot_complete_without_response(self):
        request_obj = SecondOpinionRequest.objects.create(
            requester=self.requester,
            assignee=self.specialist,
            patient=self.patient,
            diagnosis=self.transaction,
            question='Need clarification on edema extent.',
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        request_obj.refresh_from_db()
        self.assertEqual(request_obj.status, SecondOpinionRequest.STATUS_PENDING)
